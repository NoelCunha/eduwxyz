import React, { useState } from 'react';
    ...
    const BrutalCard = ({ icon: Icon, title, description, number }) => (
      <motion.div 
        ...
        transition={{ duration: 0.1 }} // Transição instantânea no hover
        className="group relative border-r border-b border-[#222] p-8 h-full flex flex-col justify-between cursor-crosshair bg-[#050505]"
      >
        <Crosshair className="top-[-8px] left-[-8px]" />
        
        <div>
          <div className="flex justify-between items-start mb-8">
            <Icon size={32} strokeWidth={1.5} className="text-[#444] group-hover:text-black transition-colors duration-100" />
      ...
      return (
        <div className="min-h-screen bg-[#050505] text-[#e5e5e5] font-mono flex flex-col relative selection:bg-neon selection:text-black overflow-x-hidden">
          ...
          <motion.div 
            ...
            className="border-b border-[#222] bg-black text-white overflow-hidden py-3 whitespace-nowrap relative z-40"
          >
            <div className="animate-marquee inline-block text-xs font-bold tracking-[0.2em]">
              <span className="mx-8">SYSTEM_STATUS: ONLINE</span>
              <span className="mx-8 text-neon">/// VIBE CODING PROTOCOL INITIATED</span>
              <span className="mx-8">AI ENGINEERING LABS</span>
              <span className="mx-8">NO_CODE_REQUIRED</span>
              <span className="mx-8">SYSTEM_STATUS: ONLINE</span>
              <span className="mx-8 text-neon">/// VIBE CODING PROTOCOL INITIATED</span>
            </div>
          </motion.div>
          ...
          <motion.header 
            ...
            className="border-b border-[#222] p-6 flex justify-between items-center sticky top-0 bg-[#050505]/90 backdrop-blur-sm z-30"
          >
            <div className="flex items-center gap-3">
              <div className="w-3 h-3 bg-neon animate-pulse"></div>
              <span className="font-bold tracking-tighter text-lg">AI_ENG_LABS</span>
            </div>
          ...
          {/* --- Hero Section --- */}
          <section className="border-b border-[#222] grid lg:grid-cols-12 min-h-[85vh]">
            
            {/* Left Content */}
            <div className="lg:col-span-7 p-8 md:p-20 flex flex-col justify-center border-b lg:border-b-0 lg:border-r border-[#222] relative">
              <Crosshair className="bottom-[-8px] right-[-8px] z-10" />
              ...
              <motion.div 
                ...
                className="mb-12"
              >
                <span className="bg-[#222] text-[#888] px-2 py-1 text-[10px] uppercase tracking-widest">
                  v2.0.4 // Stable Build
                </span>
              </motion.div>
              
              <h1 className="text-6xl md:text-8xl font-bold uppercase leading-[0.85] tracking-tighter mb-10 overflow-hidden">
                <motion.span 
              ...
              <motion.p 
                ...
                className="text-[#666] text-lg max-w-md mb-16 leading-relaxed border-l-2 border-[#222] pl-6"
              >
              ...
              >
                ...
                {!submitted ? (
                  <form onSubmit={handleSubmit} className="relative flex border border-[#333] bg-black">
                    <div className="flex items-center justify-center px-4 border-r border-[#333] text-[#444]">
                      <Terminal size={16} />
                    </div>
                    <input 
                      type="email" 
                      ...
                      className="w-full bg-transparent p-4 text-white placeholder-[#333] focus:outline-none font-bold uppercase text-sm"
                    />
                    <motion.button 
                      whileHover={{ backgroundColor: "#00FF00", color: "#000" }}
                      whileTap={{ scale: 0.95 }}
                      type="submit"
                      className="bg-white text-black px-8 font-bold uppercase transition-colors flex items-center gap-2"
                    >
                      <ArrowRight size={18} />
                    </motion.button>
                  </form>
                  ...
                  <motion.div 
                    ...
                    className="border border-neon bg-neon/10 p-4 text-neon flex items-center gap-4"
                  >
                    <Square size={16} fill="currentColor" />
                    <span className="font-bold uppercase tracking-widest">Access_Granted</span>
                  </motion.div>
                )}
              </motion.div>
            </div>
            ...
            {/* Right Visual */}
            <div className="lg:col-span-5 bg-[#080808] relative overflow-hidden flex flex-col border-b lg:border-b-0 border-[#222]">
              ...
              <div className="p-8 flex-1 flex flex-col justify-between relative z-10">
                <div className="flex justify-between items-start">
                  ...
                  <Disc className="text-[#222] animate-spin" style={{ animationDuration: '3s' }} />
                </div>
                ...
                <motion.div 
                  ...
                  initial="hidden"
                  animate="visible"
                  className="font-mono text-xs md:text-sm space-y-2 my-auto"
                >
                  <motion.div variants={itemVars} className="text-[#444] border-b border-[#222] pb-2 mb-4">
                    // AGENT_LOGS.TXT
                  </motion.div>
                  <motion.p variants={itemVars} className="text-neon">> Initializing ArchitectAgent...</motion.p>
                  <motion.p variants={itemVars} className="text-[#666]">Loading context window (128k)... OK</motion.p>
                  <motion.p variants={itemVars} className="text-[#666]">Connecting to VectorDB... OK</motion.p>
                  <motion.p variants={itemVars} className="text-white">> User Goal: "Build SaaS MVP"</motion.p>
                  <motion.p variants={itemVars} className="text-[#666]">Analyzing requirements...</motion.p>
                  <motion.p variants={itemVars} className="text-[#666]">Generating schema...</motion.p>
                ...
                <div className="grid grid-cols-3 gap-px bg-[#222] border border-[#222]">
                  {['REACT', 'PYTHON', 'SQL'].map((tech) => (
                    <motion.div 
                      ...
                      className="bg-[#080808] p-2 text-center text-[10px] text-[#666] cursor-pointer transition-colors"
                    >
                      {tech}
                    </motion.div>
                  ))}
          ...
          <section className="border-b border-[#222]">
            ...
            >
              <motion.div variants={itemVars} className="p-10 border-r border-b border-[#222] lg:col-span-3 bg-[#0a0a0a] flex flex-col md:flex-row justify-between items-end gap-6">
                <div>
          ...
          {/* --- YouTube Section --- */}
          <section className="grid lg:grid-cols-2 border-b border-[#222]">
            <motion.div 
              initial={{ opacity: 0, x: -50 }}
              whileInView={{ opacity: 1, x: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.6, ease: "circOut" }}
              className="bg-[#e5e5e5] text-black p-12 md:p-24 flex flex-col justify-center relative overflow-hidden group"
            >
              <Crosshair className="top-4 right-4 text-black" />
              
              <div className="absolute bottom-0 right-0 text-[200px] font-bold text-black opacity-5 leading-none select-none pointer-events-none">
                YT
              </div>
              
              <div className="flex items-center gap-4 mb-8">
                <div className="bg-red-600 text-white px-3 py-1 text-xs font-bold uppercase">Live</div>
                <div className="text-xs font-bold uppercase tracking-widest">@eduwxyz</div>
              </div>

              <h2 className="text-6xl font-bold uppercase tracking-tighter mb-8 leading-[0.9]">
                Conteúdo<br/>Denso
              </h2>
              <p className="text-xl mb-12 max-w-md font-medium leading-relaxed border-l-4 border-black pl-6">
              ...
              <motion.a 
                ...
                whileTap={{ scale: 0.98 }}
                className="inline-flex items-center justify-between w-full max-w-sm bg-black text-white px-8 py-6 font-bold uppercase hover:bg-red-600 transition-colors"
              >
            ...
            <div className="bg-[#0a0a0a] p-12 flex items-center justify-center border-l border-[#222] relative">
              ...
              >
                ...
                <div className="absolute top-4 left-4 right-4 flex justify-between text-[10px] text-[#666] z-20">
                ...
                <div className="absolute inset-0 flex items-center justify-center z-20">
                  <motion.div 
                    ...
                    className="w-24 h-24 border-2 border-white rounded-full flex items-center justify-center bg-black/50 backdrop-blur-sm"
                  >
          ...
          {/* --- Footer --- */}
          <footer className="p-12 bg-[#050505] text-[#444] flex flex-col md:flex-row justify-between items-end gap-12">
            <div>
              <motion.div 
                initial={{ opacity: 0 }}
              ...
              <div className="text-sm max-w-xs font-mono leading-relaxed">
